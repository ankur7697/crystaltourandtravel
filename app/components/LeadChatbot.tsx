...
export default function LeadChatbot() {
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-[80] sm:bottom-6 sm:right-6">
      <div
        className={`mb-3 w-[calc(100vw-2rem)] max-w-[370px] origin-bottom-right overflow-hidden rounded-lg border border-[#075985]/14 bg-[#f8fafc] text-[#0f172a] shadow-2xl shadow-[#0f172a]/22 transition duration-200 sm:w-[370px] ${
          isOpen
            ? "pointer-events-auto translate-y-0 scale-100 opacity-100"
            : "pointer-events-none translate-y-4 scale-95 opacity-0"
        }`}
      >
        <div className="relative overflow-hidden bg-[#0f172a] px-4 py-3.5 text-white">
          <div className="absolute inset-x-0 bottom-0 h-1 bg-[#0f766e]" />
          <div className="absolute -right-12 -top-16 h-32 w-32 rounded-full bg-[#fbbf24]/18" />
          <div className="absolute -bottom-20 left-12 h-32 w-32 rounded-full bg-[#0f766e]/16" />
          <div className="relative flex items-center justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              <div className="relative grid h-11 w-11 shrink-0 place-items-center rounded-full bg-[#f8fafc] text-base font-black text-[#075985] shadow-lg ring-2 ring-white/15">
                CT
                <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-[#0f172a] bg-[#35d07f]" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-black leading-5">CrystalTourAndTravel Support</p>
              ...
              >
                ...
                <p
                  className={`max-w-[80%] px-3 py-2 text-[13px] font-semibold leading-5 shadow-sm ${
                    message.sender === "user"
                      ? "rounded-[16px] rounded-br-md bg-[#0f766e] text-white shadow-[#0f766e]/18"
                      : "rounded-[16px] rounded-bl-md border border-[#075985]/10 bg-[#fffdf8] text-[#0f172a]"
                  }`}
                >
                  {message.text}
                </p>
            ...
            {isTyping ? (
              <div className="flex items-end gap-2">
                ...
                <div className="flex items-center gap-1 rounded-[16px] rounded-bl-md border border-[#075985]/10 bg-[#fffdf8] px-3.5 py-2.5 shadow-sm">
                  <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#0f766e]" />
                  <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#0f766e] [animation-delay:120ms]" />
                  <span className="h-1.5 w-1.5 animate-bounce rounded-full bg-[#0f766e] [animation-delay:240ms]" />
                </div>
              </div>
            ) : null}
        ...
        <div className="border-t border-[#075985]/10 bg-[#fffdf8] p-3.5 shadow-[0_-10px_24px_rgba(23,33,31,0.04)]">
          ...
          {!leadData.serviceType && !isComplete ? (
            <div className="grid grid-cols-2 gap-2">
              {serviceTypes.map((serviceType) => (
                <button
                  className="rounded-md border border-[#075985]/14 bg-[#f7f1e6] px-3 py-2.5 text-left text-[13px] font-black text-[#075985] shadow-sm shadow-[#0f172a]/4 hover:border-[#0f766e]/50 hover:bg-white hover:text-[#c94d34]"
                  key={serviceType}
          ...
          {activeStep && !isComplete ? (
            <form className="space-y-2.5" onSubmit={handleSubmit}>
              {activeStep.choices ? (
                <div className="flex flex-wrap gap-2">
                  {activeStep.choices.map((choice) => (
                    <button
                      className="rounded-full border border-[#075985]/10 bg-[#f7f1e6] px-3 py-1.5 text-[11px] font-black text-[#075985] hover:border-[#0f766e]/40 hover:bg-[#0f766e] hover:text-white"
                      key={choice}
              ...
              {error ? (
                <p className="rounded-md bg-[#0f766e]/10 px-3 py-2 text-xs font-bold text-[#b53e28]">
                  {error}
                </p>
              ) : null}
              ...
              <div className="flex gap-2">
                ...
                <button
                  className="flex-1 rounded-md bg-[#0f172a] px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-[#0f172a]/18 hover:bg-[#075985] disabled:cursor-not-allowed disabled:opacity-60"
          ...
          {isComplete ? (
            <div className="space-y-3">
              ...
              <button
                className="w-full rounded-md border border-[#075985]/16 bg-white px-4 py-2.5 text-sm font-black text-[#075985] hover:border-[#0f766e] hover:text-[#0f766e]"
                onClick={resetChat}
                type="button"
              >
      ...
      <button
        ...
        className="pointer-events-auto group relative ml-auto flex h-14 items-center gap-2.5 rounded-full bg-[#0f172a] px-4 text-sm font-black text-white shadow-xl shadow-[#0f172a]/24 hover:bg-[#075985]"
        onClick={toggleOpen}
        type="button"
      >
        <span className="absolute inset-0 -z-10 rounded-full bg-[#075985]/35 blur-lg transition group-hover:bg-[#075985]/45" />
        <span className="grid h-9 w-9 place-items-center rounded-full bg-[#f8fafc] text-[#0f766e] shadow-sm">
          CT
        </span>
...
function AgentAvatar() {
  return (
    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#0f172a] text-[10px] font-black text-[#f8fafc] shadow-sm ring-2 ring-[#f8fafc]">
      CT
    </span>
  );
}

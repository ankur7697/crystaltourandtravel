...
export default function FlightSearch() {
  ...
  return (
    <section id="flights" className="bg-[#07111f] px-4 py-14 text-white sm:px-8 sm:py-20">
      <div
      ...
      >
        ...
        >
          <p className="mb-3 text-xs font-black uppercase tracking-[0.2em] text-[#bef264] sm:mb-4 sm:text-sm">
            Travel desk
          </p>
          <h2 className="text-3xl font-black leading-tight sm:text-5xl">
            Check the essentials before a consultant finalizes your route.
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-white/68 sm:mt-6 sm:text-lg sm:leading-8">
            Use this desk to check route and stay signals. The consultant team
            can then verify supplier terms, availability, and booking steps.
          </p>
          <div className="mt-8 hidden border-l border-white/16 pl-5 text-sm font-bold leading-7 text-white/62 2xl:block">
            <p>Live flight lookup</p>
            <p>Hotel destination search</p>
            <p>Quote handoff by phone or WhatsApp</p>
          </div>
        </div>

        <div className="relative z-10 overflow-hidden rounded-[2.25rem] border border-white/12 bg-[#0b1729] text-white shadow-2xl shadow-black/25">
          <div className="grid lg:grid-cols-[210px_1fr]">
            <div className="border-b border-white/10 bg-white/[0.04] p-4 lg:border-b-0 lg:border-r lg:p-5">
              <p className="mb-4 text-[11px] font-black uppercase tracking-[0.2em] text-[#bef264]">
                Select service
              </p>
              <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
                <button
                  className={`min-h-20 rounded-[1.25rem] border p-4 text-left transition ${
                    mode === "flights"
                      ? "border-[#bef264] bg-[#bef264] text-[#07111f]"
            ...
            <div className="p-4 sm:p-7">
              <div className="mb-6 flex flex-col gap-2 border-b border-white/10 pb-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[#c75b3a]">
          ...
          {mode === "flights" ? (
            <form className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-8" onSubmit={handleSubmit}>
              ...
              <label className="min-w-0 lg:col-span-2">
                ...
                <input
                  className="h-12 w-full min-w-0 rounded-[1rem] border border-white/12 bg-white/8 px-3 text-sm font-bold text-white outline-none transition [color-scheme:dark] focus:border-[#bef264] focus:bg-white/12 focus:ring-2 focus:ring-[#bef264]/15 sm:h-14 sm:px-4"
              ...
              <button
                className="h-12 rounded-[1rem] bg-[#c75b3a] px-5 text-base font-black text-white shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:bg-[#a94a31] disabled:cursor-not-allowed disabled:translate-y-0 disabled:opacity-65 sm:h-14 sm:text-lg lg:col-span-2 lg:self-end"
                disabled={isLoading}
                type="submit"
              >
            ...
            >
              ...
              />
              <button
                className="h-11 w-full min-w-0 rounded-[1rem] bg-[#c75b3a] px-4 text-center text-sm font-black text-white shadow-lg shadow-black/20 transition hover:-translate-y-0.5 hover:bg-[#a94a31] disabled:translate-y-0 disabled:cursor-not-allowed sm:col-span-2 sm:h-12 lg:col-span-3 lg:self-end"
          ...
          {mode === "flights" && message ? (
            <div className="mt-6 overflow-hidden rounded-[1.25rem] border border-white/12 bg-white/8 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-4 px-4 py-4">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-[#bef264]">
                    Flight availability
                  </p>
              ...
              <button
                className="rounded-full border border-white/16 bg-white/10 px-3 py-2 text-sm font-black text-white hover:border-[#bef264] hover:text-[#bef264]"
                onClick={handleClear}
                type="button"
              >
                Clear
            ...
            <div className="mt-6 overflow-hidden rounded-[1.25rem] border border-white/12 bg-white/8 shadow-sm">
              <div className="grid gap-4 p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                <div>
                  ...
                  {hotelResults.length ? (
                    <div className="mt-3 flex flex-wrap gap-2 text-xs font-black text-white/72">
                      <span className="rounded-full bg-white/10 px-3 py-2 shadow-sm">
                ...
                {hotelResults.length ? (
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="rounded-[1rem] bg-[#bef264] px-4 py-3 text-[#07111f] shadow-lg shadow-black/15">
          ...
          {mode === "flights" && results.length ? (
            <div className="mt-6 space-y-3">
              {results.map((result) => (
                <div
                  className="grid overflow-hidden rounded-[1.25rem] border border-white/12 bg-white text-[#0f172a] shadow-sm lg:grid-cols-[220px_1fr_auto] lg:items-stretch"
                  key={`flight-${result.id}`}
                >
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#0f172a]/10 bg-[#f7f2ea] px-4 py-3 lg:block lg:border-b-0 lg:border-r lg:py-5">
                  ...
                  <div className="grid gap-4 p-4 sm:grid-cols-[1fr_auto_1fr] sm:items-center lg:px-6">
                    ...
                    <div className="hidden min-w-24 items-center justify-center sm:flex">
                      <div className="h-px w-10 bg-[#0f172a]/20" />
                      <div className="grid h-9 w-9 place-items-center rounded-full bg-[#0f172a] text-xs font-black text-white">
                        To
                      </div>
                      <div className="h-px w-10 bg-[#0f172a]/20" />
                    </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#0f172a]/10 px-4 py-3 lg:block lg:border-l lg:border-t-0 lg:py-5">
...
function AirportBlock({
  ...
  return (
    <div className="min-w-0">
      <p className="text-xs font-black uppercase tracking-[0.14em] text-[#c75b3a]">
        {label}
      </p>
      <div className="mt-2 flex items-baseline gap-3">
        <p className="text-3xl font-black text-[#172554]">{code}</p>
  ...
  return (
    <div className="block min-w-0 lg:col-span-2">
      ...
      <input
        aria-label={label}
        autoComplete="off"
        className="pointer-events-auto h-12 w-full touch-manipulation rounded-[1rem] border border-white/12 bg-white/8 px-3 text-base font-black text-white outline-none transition placeholder:text-white/35 focus:border-[#bef264] focus:bg-white/12 focus:ring-2 focus:ring-[#bef264]/15 sm:h-14 sm:px-4 sm:text-sm"
        id={inputId}
        onBlur={() => {
      ...
      {isFocused && suggestions.length ? (
        <div className="mt-2 max-h-64 overflow-y-auto rounded-[1rem] border border-white/12 bg-[#101f35] p-2 shadow-lg shadow-black/20">
          {suggestions.map((airport) => (
            ...
            >
              ...
              <span className="shrink-0 rounded-full bg-[#bef264] px-3 py-2 text-sm font-black text-[#07111f]">
                {airport.code}
              </span>
            </button>
          ))}
...
function HotelDestinationInput({
  ...
  return (
    <div className="relative block min-w-0 sm:col-span-2 lg:col-span-4">
      ...
      <input
        autoComplete="off"
        className="h-12 w-full rounded-[1rem] border border-white/12 bg-white/8 px-3 text-base font-black text-white outline-none transition placeholder:text-white/35 focus:border-[#bef264] focus:bg-white/12 focus:ring-2 focus:ring-[#bef264]/15 sm:h-14 sm:px-4"
        id="hotel-place"
      ...
      {isFocused && value.trim().length >= 2 ? (
        <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-30 max-h-64 overflow-y-auto rounded-[1rem] border border-white/12 bg-[#101f35] p-2 shadow-2xl shadow-black/25">
          {isLoading ? (
            ...
            places.map((place) => (
              <button
                className="flex w-full items-center justify-between gap-3 rounded-[0.75rem] px-3 py-2.5 text-left hover:bg-white/8"
                key={place.placeId}
                onMouseDown={(event) => event.preventDefault()}
                onPointerDown={(event) => {
                  event.preventDefault();
              ...
              >
                ...
                <span className="shrink-0 rounded-full bg-[#bef264] px-2.5 py-1.5 text-[11px] font-black uppercase text-[#07111f]">
                  {place.type || "place"}
                </span>
              </button>
            ))
...
function NumberInput({
  ...
  return (
    <label className="min-w-0 lg:col-span-2">
      ...
      <input
        className="h-12 w-full rounded-[1rem] border border-white/12 bg-white/8 px-3 text-base font-bold text-white outline-none transition focus:border-[#bef264] focus:bg-white/12 focus:ring-2 focus:ring-[#bef264]/15 sm:h-14 sm:px-4"
        min={min}
...
function HotelResultCard({
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-[#0f172a]/10 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl hover:shadow-[#0f172a]/10">
      <div
        className="relative min-h-[190px] bg-[#172554]/10 bg-cover bg-center"
        style={{
      ...
      >
        <div className="absolute left-3 top-3 rounded-md bg-white/94 px-3 py-2 text-xs font-black text-[#172554] shadow-sm">
          {hotel.starRating ? `${Math.round(hotel.starRating)} star` : "Rating TBA"}
        </div>
        {typeof hotel.refundable === "boolean" ? (
          <div className="absolute bottom-3 left-3 rounded-md bg-[#0f172a]/86 px-3 py-2 text-xs font-black text-white">
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs font-black text-[#475569]">
          <span className="rounded-md bg-[#f7f2ea] px-3 py-2">
            {hotel.roomName || "Room details available"}
          </span>
        </div>
        <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-[#0f172a]/10 pt-4">
          <HotelPriceBlock hotel={hotel} />
          <button
            className="rounded-md bg-[#0f172a] px-4 py-3 text-sm font-black text-white hover:bg-[#172554]"
...
function HotelDetailsModal({
  ...
  const whatsappText = encodeURIComponent(
    `Hi, I want details for ${hotel.name} on CrystalTourAndTravel.`,
  );

  return (
    <div className="fixed inset-0 z-[90] flex items-end bg-[#0f172a]/62 px-4 py-4 backdrop-blur-sm sm:items-center sm:justify-center">
      <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white text-[#0f172a] shadow-2xl">
        <div
          className="relative min-h-[220px] bg-[#172554]/10 bg-cover bg-center sm:min-h-[320px]"
          style={{
        ...
        >
          <button
            ...
            className="absolute right-4 top-4 grid h-10 w-10 place-items-center rounded-full bg-white/92 text-lg font-black text-[#0f172a] shadow-lg hover:bg-white"
            onClick={onClose}
            type="button"
          >
            x
        ...
        <div className="p-5 sm:p-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h3 className="mt-2 text-3xl font-black leading-tight text-[#0f172a]">
                {hotel.name}
              </h3>
          ...
          <div className="mt-6 border-y border-[#0f172a]/10 py-5">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-[#64748b]">
              Approx. total
            </p>
            <p className="mt-2 text-5xl font-black leading-none text-[#c75b3a]">
              <span className="mr-2 align-middle text-base text-[#a94a31]">
                {hotel.currency || "USD"}
              </span>
          ...
          <div className="mt-6 rounded-lg bg-[#f7f2ea] p-4">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            ...
            <button
              className="rounded-md border border-[#0f172a]/15 px-5 py-3 text-sm font-black text-[#0f172a] hover:border-[#c75b3a] hover:text-[#c75b3a]"
              onClick={onClose}
              type="button"
            >
              Back to Results
...
function HotelDetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[#0f172a]/10 bg-[#fbfaf6] p-4">
      <p className="text-xs font-black uppercase tracking-[0.12em] text-[#c75b3a]">
        {label}
      </p>
      <p className="mt-2 text-sm font-black leading-6 text-[#0f172a]">
        {value}
      </p>
    </div>
  );
...
function HotelPriceBlock({ hotel }: { hotel: HotelResult }) {
  return (
    <div>
      ...
      <p className="mt-1 text-3xl font-black leading-none text-[#c75b3a]">
        <span className="mr-1 align-middle text-xs text-[#a94a31]">
          {hotel.currency || "USD"}
        </span>
        {formatHotelPrice(hotel.price)}
      </p>
...
function getStatusClass(value: string) {
  ...
  if (normalized === "active") {
    return "bg-[#172554]/10 text-[#172554]";
  }
  ...
  if (normalized === "cancelled" || normalized === "incident") {
    return "bg-[#c75b3a]/12 text-[#b43d27]";
  }
